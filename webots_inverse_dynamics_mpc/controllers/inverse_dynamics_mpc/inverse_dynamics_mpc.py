"""Webots controller: nonlinear inverse-dynamics MPC for a single inverted pendulum."""
...
def main():
    ...
    # Direct force control. Webots bypasses its internal PID for setForce().
    ...
    print("\n=== Webots inverse-dynamics MPC: single inverted pendulum ===")
    ...
    with log_path.open("w", newline="") as f:
        ...
        while robot.step(time_step_ms) != -1:
            ...
            # Solve at 50 Hz; hold the first optimized force between MPC updates.
            if t - last_solve_time >= MPC_PERIOD_S - 0.5 * dt:
                ...
                passive_residual = result.unactuated_residual
                last_solve_time = t
            ...
            writer.writerow(
                [
                    ...
                    f"{first_a[0]:.9f}",
                    f"{first_a[1]:.9f}",
                    f"{passive_residual:.12e}" if np.isfinite(passive_residual) else "",
                ]
            )
            f.flush()
