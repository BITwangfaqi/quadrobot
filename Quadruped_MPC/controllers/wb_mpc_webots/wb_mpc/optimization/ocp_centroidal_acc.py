...
class OCPCentroidalAcc(OCP):
    def __init__(self, robot, nodes, tau_nodes, warm_start, include_base=False):
        super().__init__(robot, nodes, tau_nodes, warm_start)

        # Dynamics
        self.dyn = DynamicsCentroidalAcc(self.model, self.mass, self.ee_frames)
    ...
    def setup_dynamics_constraints(self, i):
        ...
        if self.include_base:
            ...
            gaps = self.dyn.dynamics_gaps()(q, v, a, forces)
            self.opti.subject_to(gaps == [0] * 6)

        if i < self.tau_nodes:
            # Torque limits (estimate through contact Jacobian)
            tau_j = self.dyn.tau_estimate()(q, forces)
    ...
    def get_a(self, i):
        if self.include_base:
            ...
            v = self.get_v(i)
            forces = self.get_forces(i)
            a_b = self.dyn.base_acc_dynamics()(q, v, a_j, forces)
